<template>
  <div id="projectListBalance" style="height:100%;">
          ...
          <li v-for="(item,index) in projectList" @click="toggleButton(item)" :key="index">
            ...
            <transition name="slide-fade">
              <div v-show="item.show" class="itemButtonBox">
                ...
                <x-button mini class="itemButtonBoxItem" @click.native="hrefClick(1,item)">查看发票台账</x-button>
              </div>
            </transition>
          </li>
          <li>
...
export default {
  ...
  methods: {
    onloadprojectList() {
      ...
      this.$fetch('/balance/project-list.do?pageNum=' + this.pageNum + '&pageSize=10').then(response => {
        if (response.status === 0) {
